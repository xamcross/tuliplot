...
- **Testcontainers on this host:** Docker Desktop serves the `desktop-linux` context; `~/.testcontainers.properties` is set to `docker.host=npipe:////./pipe/dockerDesktopLinuxEngine` (fallback: prefix gradle with `DOCKER_HOST=npipe:////./pipe/dockerDesktopLinuxEngine`).
