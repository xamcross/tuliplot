...
- **Testcontainers 2.x coordinates** (BOM-managed by Boot 4.1): use `org.testcontainers:testcontainers-junit-jupiter` and `org.testcontainers:testcontainers-mongodb` — the plans' `org.testcontainers:junit-jupiter` / `:mongodb` do NOT resolve. The `org.testcontainers.*` import packages are unchanged. Testcontainers tests need the Docker daemon running.
- **`TestRestTemplate` was removed in Boot 4.x.** For full-boot HTTP tests use `@LocalServerPort` (`org.springframework.boot.test.web.server.LocalServerPort`) + Spring `RestClient`. Reviewers must not flag this substitution.
- **More relocated test slices (Boot 4.1):** `@DataMongoTest` = `org.springframework.boot.data.mongodb.test.autoconfigure.DataMongoTest` + dep `spring-boot-data-mongodb-test`; `@AutoConfigureMockMvc` = `org.springframework.boot.webmvc.test.autoconfigure.AutoConfigureMockMvc`.
- **Jackson 3:** the JSON mapper is `tools.jackson.databind.ObjectMapper` (NOT `com.fasterxml.jackson.databind.ObjectMapper`). `spring-boot-starter-validation` is on the classpath (added Plan 02 T3).
- **Testcontainers on this host:** run Testcontainers gradle tests as `DOCKER_API_VERSION=1.44 ./gradlew test ...` (docker-java negotiates 1.32, below the daemon minimum). Docker Desktop serves the `desktop-linux` context; `~/.testcontainers.properties` is set to `docker.host=npipe:////./pipe/dockerDesktopLinuxEngine` (fallback: also prefix `DOCKER_HOST=npipe:////./pipe/dockerDesktopLinuxEngine`).
- **Node for the frontend:** system Node (v24.13) is below Angular 22's minimum; a compatible Node v22.22.3 is provisioned at `C:\Users\xamcr\.dashdash-tooling\node-v22.22.3-win-x64` — prepend its dir to PATH for all frontend commands. CI uses `actions/setup-node` @ node 22, so CI is unaffected. The Angular app is already scaffolded in `frontend/` (Plan 01 Task 5) — later plans add to it, they do not re-scaffold.
