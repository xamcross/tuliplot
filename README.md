...
2. Fly.io: `fly apps create api-tuliplot`; copy secrets from the old app (`MONGODB_URI` — switch the DB name to /tuliplot, `GOOGLE_CLIENT_ID/SECRET`, `FREEMIUS_*`, `COOKIE_DOMAIN=tuliplot.com` (no leading dot — RFC 6265 rejects it and Tomcat throws; the dotless form covers subdomains), `COOKIE_SECURE=true`, `CORS_ALLOWED_ORIGINS=https://tuliplot.com`, `OAUTH2_SUCCESS_URL=https://tuliplot.com/app`); `fly deploy`; add DNS `api.tuliplot.com` → Fly cert.
...
4. Freemius dashboard: go to Webhooks and add the endpoint `https://api.tuliplot.com/api/v1/billing/webhook` (verify path against FreemiusWebhookController); Freemius signs each event with the product's secret key, already set in `FREEMIUS_SECRET_KEY`.
